<script setup lang="ts">
  ...
  // (A) propsで必要なdefinePropsを追加
  //import { defineProps } from 'vue';
  // (A) propsを定義して受け取る
  //const props = defineProps<{ title: string }>();

  // (B) propsで必要なdefinePropsを追加
  interface Props {
    title: string;
  }
  // (B) propsを定義して受け取る
...
<template>
  <!-- (A)propsで受け取ったタイトルを表示 -->
  <!-- <H2>{{ props.title }}</H2> -->

  <!-- (B)propsで受け取ったタイトルを表示 -->
  <H2>{{ title }}</H2>
